...
//           // Push each object into the list
//           list.push({'charity_id': sampleData.charity_id[i],'charity_name': sampleData.charity_name[i], 'overall_score': sampleData.scores[i], 'total_contributions':sampleData.total_contributions[i]});
//         }

//         // Sort function of objects by samples values in array 
//         console.log(list.sort((a, b) => parseInt(b.overall_score) - parseInt(a.overall_score)));

//         var charity_id= list.slice(0,15).map(record => "ID" + record.charity_id.toString());
//         var values = list.slice(0,15).map(record => record.overall_score);
//         var total_contributionss = list.slice(0,15).map(record =>  record.total_contributions );
//         var charity_name= list.slice(0,15).map(record =>  record.charity_name );
  
//     var trace1 = [{
//       x: total_contributionss,
//       y: charity_id,
//       hovertext:charity_name,
//       type: "bar",
//       orientation: "h"
//     }];

//     var layout = {
//       title:"<b> Top 15 charity Bar Chart </b>",
//       yaxis:{autorange:'reversed'},
